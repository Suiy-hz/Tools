...
public class IDCardGenerator {
    ...
    /**
     * 身份证号的结构：公民身份号码是特征组合码，由十七位数字本体码和一位校验码组成。排列顺序从左至右依次为：六位数字地址码，
    ...
    /**
     * 生成有效期限：当前日期的前10年-当前日期的后10年
     * Valid Through
     */
    public static String generateValidPeriod() {
        Date beginDate = TimeUtils.getSpecificYeasGapDate(TimeUtils.getNow(), -10);
        Date endDate = TimeUtils.getSpecificYeasGapDate(TimeUtils.getNow(), 10);
        return TimeUtils.dateFormat2(beginDate) + "-" + TimeUtils.dateFormat2(endDate);
    }
    ...
    public static String generateIDCard() {
        ...
        String areaCode = code.keySet().toArray(new String[0])[RandomUtils
                ...
                "0");
        String birthday = TimeUtils.dateFormat2(TimeUtils.randomDate());
